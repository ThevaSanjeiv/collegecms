...
def getLoginPage(req):
    try:
        ...
        if user_session and role_session:
            ...
            elif role_session=="admin":
                return redirect("AdminDashboard")

        if req.method == "POST":
            reqMethod=req.POST
            email = reqMethod.get("email").strip()
            password = reqMethod.get("password").strip()
            ...
            if user:
                if user.get("password") == password:
                    ...
                    return render(req, "portal/login.html", {"error": "Incorrect password"})
            else:
                return render(req, "portal/login.html", {"error": "Email not registered"})

        return render(req, "portal/login.html")

    except Exception as e:
        print(f"Error while checking: {e}")
        return render(req, "portal/login.html", {"error": "Error occurred while logging in"})

def AdminDashboard(req):
    return render(req,"portal/admin.html")
